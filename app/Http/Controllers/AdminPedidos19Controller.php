<?php namespace App\Http\Controllers;
	...
	class AdminPedidos19Controller extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col = [];
			$this->col[] = ["label"=>"Cliente Id","name"=>"cliente_id","join"=>"cliente1,NOMBRE"];
			$this->col[] = ["label"=>"Numero Pedido","name"=>"numero_pedido"];
			$this->col[] = ["label"=>"Subtotal","name"=>"subtotal"];
			$this->col[] = ["label"=>"Iva","name"=>"iva"];
			$this->col[] = ["label"=>"Total","name"=>"total"];
			# END COLUMNS DO NOT REMOVE THIS LINE

			# START FORM DO NOT REMOVE THIS LINE
			$this->form = [];
			$this->form[] = ['label'=>'Cliente Id','name'=>'cliente_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'cliente1,id'];
			$this->form[] = ['label'=>'Numero Pedido','name'=>'numero_pedido','type'=>'number','validation'=>'required|integer|min:0','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Subtotal','name'=>'subtotal','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Iva','name'=>'iva','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
            ...
            $columns[] = ['label'=>'Producto',
                          'name'=>'producto_id',
                          'type'=>'datamodal',
                          'datamodal_table'=>'producto',
                          'datamodal_columns'=>'descripcion,precio',
                          'datamodal_select_to'=>'precio_unit:precio',
                          'datamodal_where'=>'',
                          'datamodal_size'=>'large'];
            
            $columns[] = ['label'=>'Price','name'=>'precio_unit','type'=>'number','required'=>true];
            $columns[] = ['label'=>'QTY','name'=>'cantidad','type'=>'number','required'=>true];

            $columns[] = ['label'=>'Sub Total','name'=>'precio_total','type'=>'number','formula'=>"[cantidad] * [precio]","readonly"=>true,'required'=>true];
            $this->form[] = ['label'=>'Orders Detail','name'=>'detalle_pedidos','type'=>'child','columns'=>$columns,'table'=>'detalle_pedidos','foreign_key'=>'pedidos_id'];
	        ...
	        */
	        $this->addaction = array([
	            'label'=>'IMPRIMIR',
                'url'=>CRUDBooster::adminPath($slug='pedido/pedido_print').'/[id]'
            ]);
	    ...
	    */
        ...
        {
            $pedido = Pedido::select("numero_pedido")->orderBy("numero_pedido","desc")->first();

            $this->cbView("pedidos.crear_pedido",["pedido"=>$pedido]); // TODO: Change the autogenerated stub
        }
